...
#figure(image("../static/p-value-significance-level/p-value-significance-level_files/p-value-significance-level_3_1.png", alt: "png"),
  caption: [
    png
  ]
)
...
#figure(image("../static/p-value-significance-level/p-value-significance-level_files/p-value-significance-level_11_1.png", alt: "png"),
  caption: [
    png
  ]
)
...
In this example, we turn to Wilks' Theorem, which states that the test statistic $2 ln frac(max L_(upright("alter")), max L_(upright("null")))$ has its null hypothesis as $chi_k^2$ distribution where $k$ is the difference in the dimensions of the two hypotheses.

So what is $k$ in our example? Note that our hypotheses are of the forms

$ L_"null" &= a, \
  L_"alter" &= a + b dot G(M, sigma = 0.05). $
