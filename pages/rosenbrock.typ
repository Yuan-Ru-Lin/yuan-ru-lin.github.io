...
A heuristic method is Gradient Descent (#strong[GD]), which is essentially the very start of every machine learning course. However, there are other modified methods such as Gradient Descent with Classic Momentum (#strong[GDM]), Nesterov Accelerated Gradient (#strong[NAG]), Root-Mean-Square Propagation (#strong[RMSProp]), and Adaptive Momentum Estimation (#strong[ADAM]).

In this notebook, we examine how each method mentioned above perform on a simple function named Rosenbrock function.

== Disclaimer
<disclaimer>
This is the first homework for the course #emph[Machine Learning and Manybody Physics] by Prof.~Ying-Jer Kao, National Taiwan University.
...
#image("../static/rosenbrock/index_7_1.png")
...
= Modeling
<modeling>
We define the gradient of $f\,med nabla f$ below:
...
<starting-on-the-upper-plane>
Here we observe how each method approaches to the minimum from $theta_0 =\(0\,5\)$.

```python
plot(0.001, 1000, (0., 5.))
```

#image("../static/rosenbrock/index_19_1.png")

In this case, we saw:

- #strong[GDM] has a benefit of early arriving despite the route is messy.
- #strong[NAG] has much less curvature than #strong[GDM] has, but still it can have more variation than pure #strong[GD] does.
- #strong[RMSProp] and #strong[ADAM] don't behave as well as in the lecture since they require more iteration.

== Starting in the valley
<starting-in-the-valley>
Here we observe how each method approaches to the minimum from $theta_0 =\(2\,4\)$.

```python
plot(0.001, 1000, (2., 4.))
```

#image("../static/rosenbrock/index_23_1.png")

In this case, we saw #strong[GDM] still arrived the minimum earlier than any other method. What's noteworthy is that #strong[NAG] had a little oscillation at the start (but later converged anyway); it resembled a ball placed at a slightly tilted plane.

== Starting from the wall
<starting-from-the-wall>
Here we observe how each method approaches to the minimum from $theta_0 =\(2.75\,3\)$.

Note that I shrank the learning rate $eta$ to $0.0005$ and doubled the iteration here; otherwise #strong[GDM] would blow up.

```python
plot(0.0005, 2000, (2.75, 3))
```

#image("../static/rosenbrock/index_27_1.png")

In this case, we saw:

- #strong[GDM] again arrived the minimum while others didn't;
- #strong[NAG] resembled #strong[GD] at first, became like #strong[GDM] in the middle, and then went back being #strong[GD] at the end;
- #strong[RMSProp] and #strong[ADAM] didn't have enough iteration.
...
#image("../static/rosenbrock/index_31_1.png")

In this case, we saw:

- #strong[GD] and #strong[NAG] had trouble converging to the minimum for the learning rate was too large;
- #strong[GDM], on the other hand, had no trouble converging;
- #strong[RMSProp] and #strong[ADAM] converged slowly.

= Summary
<summary>
The Rosenbrock function under the given setting is a rather simple function. In such simple case, #strong[GDM] and #strong[NAG] or even #strong[GD] have performance better than #strong[RMSProp] and #strong[ADAM] do. However, in reality, it's often not the case and the latter two will would have more advantages.

= Reference
<reference>
+ #link("https://gitlab.com/yingjerkao/mlphysics")[Notebooks for "A high bias low-variance introduction to Machine Learning for physicists."]
+ #link("https://hackmd.io/@allen108108/H1l4zqtp4")[Adagrad、RMSprop、Momentum and Adam -- 特殊的學習率調整方式]
+ #link("https://www.youtube.com/watch?v=CXgbekl66jc&list=PLJV_el3uVTsPy9oCRY30oBPNLCo89yu49")[ML Lecture, Hung-yi Lee]
