...
= Tools#footnote[It's assumed that you've already imported the familiar modules i.e.~`import numpy as np; import matplotlib.pyplot as plt; import scipy.stats as st`.]
<tools1>
...
#image("../static/inference-mle/Figure_1.png")
...
== Log-likelihood
<log-likelihood>
- Generate 100 random variates from an Exponential distribution, $upright(E x p o n)\(theta = 2\)$, where $theta$ denotes its lifetime.
- Define a function `n2ll` of `theta`, which stands for $- 2 ln L\(theta\)$, with the aid of `np.sum`, `np.log`.
- Plot $- 2 ln L\(theta\)$ versus $theta in\[1\,3\]$ with the aid of `np.vectorize`.

== Point Estimation
<point-estimation>
- Determine the estimate, $hat(theta)$, by minimizing $- 2 ln L\(theta\)$ with respect to $theta$ with the aid of `opt.minimize`.

== Interval Estimation
<interval-estimation>
- Plot a vertical line passing $- 2 ln L_(upright(m a x))$.
- Plot a horizontal line $y = - 2 ln L_(upright(m a x)) + 1$.
- Construct an interval $\[theta_(upright(l o w))\,theta_(upright(h i g h))\]$ by finding roots of $f\(theta\)= - 2 ln L\(theta\)-\[- 2 ln L_(upright(m a x)) + 1\]$ with the aid of `opt.fsolve`.
- Does the true value fall within the interval? Yes or no?

== Verification and Practice
<verification-and-practice>
- Repeat the above steps 100 times. How many times do the true values fall within the intervals? That is, what's the probability of the statement "The true value falls within this interval that I construct by finding $theta$ such that $- 2 Delta ln L\(theta\)= 1$" being true?
